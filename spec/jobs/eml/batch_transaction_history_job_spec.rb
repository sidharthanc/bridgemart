describe EML::BatchTransactionHistoryJob do
  describe 'perform' do
    it 'enqueues a transaction job for each active card' do
      expect { subject.perform }.to enqueue_job(EML::TransactionHistoryJob).exactly(Code.with_card_type(:eml).count)
    end
    ...
    context 'with missing external_id for code' do
      ...
      it 'does not enqueue a transaction job' do
        expect { subject.perform }.to_not enqueue_job(EML::TransactionHistoryJob)
      end
    end
